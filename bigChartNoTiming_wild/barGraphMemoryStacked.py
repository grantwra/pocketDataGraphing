...
if sys.argv[1] == 'CPU':
	...
	ax.set_title('Runtime/CPU Time (Memory) for YCSB Workload ' + sys.argv[2],fontsize=20, fontweight='bold')
	...
	#labels = [ 'BDB100', '', 'H2 Op', 'H2 Burst','H2 End' ]
...
#	ax.set_xticklabels(('Default SQL', 'Default BDB', 'BDB 100MB Cache', 'SQL50%', 'G5'))
	...
	plt.show()
	fig15.savefig('Graphs/stacked2/' + sys.argv[1] + '_' + sys.argv[2])
	
	saveinfo = 'Graphs/stacked2/info/' + sys.argv[1] + '_' + sys.argv[2]
	best1 = 5000000.0
	bestint1 = 0
...
#               	ax.set_xticklabels(('Default SQL', 'Default BDB', 'BDB 100MB Cache', 'SQL50%', 'G5'))
			...
			fig15 = plt.gcf()
                	plt.show()
			fig15.savefig('Graphs/newIO/mem/' + sys.argv[1] + '_' + sys.argv[2])

			saveinfo = 'Graphs/newIO/mem/info/' + sys.argv[1] + '_' + sys.argv[2]
			bestio = 500000000
