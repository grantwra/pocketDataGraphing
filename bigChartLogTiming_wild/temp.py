...
path_to_default = 'configs/info/' #sql and bdb
path_to_bdb100_cache = 'configs/info/' #bdb100

default_sql = path_to_default + sys.argv[2] + '_SQL'
default_bdb = path_to_default + sys.argv[2] + '_BDB'
bdb100 = path_to_bdb100_cache + sys.argv[2] + '_BDB100'
...
if sys.argv[1] == 'CPU':
                ...
                with open(name,'r') as log:
                        for line in log:
                                ...
                                if 'Waiting' in line:
                                        ...
                                        numFinal = num.replace('\n','')
                                        #print numFinal
                                        waiting_list.append(float(numFinal))

	
	count = 0
	for i in cpu_list:
		waiting_list[count] += i
		count += 1
	'''
	sleep_list = []
	for i in waiting_list:
		sleep_list.append(i + 1800)
	'''
	#print 'goodJOb!'
	#for x in cpu_list:
	#	print x
	...
	#ax.bar(ind,sleep_list,1, alpha = 1,color='g')
	ax.bar(ind,waiting_list,1, alpha = 1,color='y')
	barlist = ax.bar(ind,cpu_list,1, alpha = 1)
	#ax.bar(ind,run_list,1, alpha = 0.5)
	#ax.bar(0,0,1,alpha = 0.5, color = 'g',label = 'Best Case')
	ax.bar(0,0,1,alpha = 1, color = 'r', label = 'App off core for other reason')
	#ax.bar(0,0,1,alpha = 1, color = 'g', label='Sleeping due to benchmark')
	ax.bar(0,0,1,alpha = 1, color = 'y', label = 'Time waiting for an IO')
	...
	ax.set_title('Runtime/CPU Time for YCSB Workload ' + sys.argv[2] + ' (No Timing)',fontsize=20, fontweight='bold')
...
#	ax.set_xticklabels(('Default SQL', 'Default BDB', 'BDB 100MB Cache', 'SQL50%', 'G5'))
	...
	plt.show()
	fig15.savefig('Graphs/Stacked' + '_' + sys.argv[2])
	
	saveinfo = 'Graphs/info/CPU' + '_' + sys.argv[2]
	best1 = 5000000.0
	bestint1 = 0
	...
	saveinfo = 'Graphs/info/RUN_' + sys.argv[2]
	best1 = 5000000.0
	bestint1 = 0
	secondbest = 5000000.0
	secondbestint = 0
	...
	for name in fileList:
    	...
    	saveinfo = 'Graphs/info/IO' + '_' + sys.argv[2]
	bestio = 500000000
...
#               	ax.set_xticklabels(('Default SQL', 'Default BDB', 'BDB 100MB Cache', 'SQL50%', 'G5'))
			...
			fig15 = plt.gcf()
                	plt.show()
			fig15.savefig('Graphs/newIO/' + sys.argv[1] + '_' + sys.argv[2])

			saveinfo = 'Graphs/newIO/info/' + sys.argv[1] + '_' + sys.argv[2]
			bestio = 500000000
			secondbestio = 5000000000
			bestreads = 50000000000
			secondbestreads = 500000000
