...
if sys.argv[1] == 'CPU':
	...
	ax.set_title('CPU time (Memory) for YCSB Workload ' + sys.argv[2],fontsize=20, fontweight='bold')
...
#	ax.set_xticklabels(('Default SQL', 'Default BDB', 'BDB 100MB Cache', 'SQL50%', 'G5'))
	...
	plt.show()
	fig15.savefig('Graphs/memory/' + sys.argv[1] + '_' + sys.argv[2])
	
	saveinfo = 'Graphs/memory/info/' + sys.argv[1] + '_' + sys.argv[2]
	best1 = 5000000.0
	bestint1 = 0
	...
	if sys.argv[1] == 'RUN':
		...
		ax.bar(0,0,1,alpha = 0.5, color = 'g',label = 'Best Case')
	        ...
	        #barlist[8].set_color('r')
		ax.set_title('Total Runtime (Memory) for YCSB Workload ' + sys.argv[2],fontsize=22)
