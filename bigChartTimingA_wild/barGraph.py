...
if sys.argv[1] == 'CPU':
	...
	ind = np.arange(N)
	fig, ax = plt.subplots()
	barlist = ax.bar(ind,cpu_list,1, alpha = 0.5)
	...
	ax.set_title('CPU time for YCSB Workload ' + sys.argv[2],fontsize=20, fontweight='bold')
	ax.set_ylabel('CPU Time (ms)',fontsize=20, fontweight='bold')
	...
	labels = [ 'Default SQL', 'Default BDB', 'BDB 100MB Cache', 'SQL heap 50%', 'BDB heap 50%', 'SQL heap 25%','BDB heap 25%' , 'H2 sync query', 'H2 sync burst','H2 sync end' ]
	labels = [ '\n'.join(wrap(l, 50)) for l in labels ]
	ax.set_xticklabels(labels,ha='right', fontweight='bold')
...
#	ax.set_xticklabels(('Default SQL', 'Default BDB', 'BDB 100MB Cache', 'SQL50%', 'G5'))
	...
	plt.show()
	fig15.savefig('Graphs/pdf/' + sys.argv[1] + '_' + sys.argv[2])
	
	saveinfo = 'Graphs/info/' + sys.argv[1] + '_' + sys.argv[2]
	best1 = 5000000.0
	bestint1 = 0
	...
	if sys.argv[1] == 'RUN':
		...
		ax.bar(0,0,1,alpha = 0.5, color = 'g',label = 'Best Case')
	        ...
	        #barlist[8].set_color('r')
		ax.set_title('Total Runtime for YCSB Workload ' + sys.argv[2],fontsize=22)
...
#       	ax.set_xticklabels(('Default SQL', 'Default BDB', 'BDB 100MB Cache', 'SQL50%', 'G5'))
		...
		if sys.argv[1] == 'IO':
			...
			ax.set_title('Total IO events for YCSB Workload ' + sys.argv[2],fontsize=22)
